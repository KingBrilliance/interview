...
/*
	这里的实现比较粗糙，但是作为一个 demo 理解 di 容器也足够了，和 dig 相比还缺少很多东西，并且有许多的问题，例如 依赖关系，一种类型如果有多个 provider 如何处理等等等等。
	可以看到我们总共就三个函数
	1).Provide: 获取对象工厂，并且使用一个 map 将对象工厂保存
	2).Invoke: 执行入口
	3).buildParam: 核心逻辑，构建参数
		1.从容器中获取 provider
...
func (c *Container) Provide(constructor interface{}) error {
	...
	if v.Kind() != reflect.Func {
		return fmt.Errorf("builder must be a func")
	}
